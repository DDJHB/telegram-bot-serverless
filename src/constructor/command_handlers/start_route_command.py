...
def handler(data: dict, chat_state: dict):
    ...
    # 1. get response from tg when the idiot presses on the keyboard :(
    # 2. based on the route get passengers
    # 3. send a keyboard to ALL passengers with "YES"/"NO" options
    # 4. accept the response from TG
    # 5. if after 10 mins less than 50% respond --> then, let the driver know that they didnt vote and do not start the ride
    # 6. if more than 50% respond within 15 mins --> then start the route

    # write to db that the route has started in route table ???
